...
export function useExpiryAlerts() {
  ...
  const checkExpiringSubscriptions = () => {
    ...
    const expiring = visitors
      ...
      .map(visitor => {
        const expiryDate = addMonths(new Date(visitor.start_date), visitor.duration);
    ...
    if (expiring.length > 0 && (now - lastAlertTime) > oneHour && !alertDismissed) {
      ...
      if (expiringToday.length > 0) {
        message = `${expiringToday.length} subscription${expiringToday.length > 1 ? 's' : ''} expire today!`;
      } else if (expiringTomorrow.length > 0) {
        message = `${expiringTomorrow.length} subscription${expiringTomorrow.length > 1 ? 's' : ''} expire tomorrow!`;
      } else if (expiringIn2Days.length > 0) {
        message = `${expiringIn2Days.length} subscription${expiringIn2Days.length > 1 ? 's' : ''} expire in 2 days!`;
      } else {
        message = `${expiringIn3Days.length} subscription${expiringIn3Days.length > 1 ? 's' : ''} expire in 3 days!`;
      }
